# P04 — Tokenizer from scratch

**Goal:** Train, test, version, and freeze a project-owned BPE tokenizer using training data only.
...
Tokens, vocabulary, BPE, special tokens, tokenizer leakage, and encode/decode round trips.
...
1. Implement BPE training using TRAIN text only.
2. Save tokenizer files plus config/hash/provenance.
3. Test empty strings, punctuation, numbers, Unicode, long text, and special tokens.
4. Inspect token-length distribution/pathological fragmentation.
5. Freeze the tokenizer artifact for downstream baseline comparisons.

## Operator commands

```bash
hai tokenizer train --config configs/tokenizers/bpe-8192.yaml --dataset-manifest data/manifests/tinystories-smoke.yaml
hai tokenizer inspect --tokenizer artifacts/tokenizers/bpe-8192-v1 --text 'The dog walked home.'
hai tokenizer verify --tokenizer artifacts/tokenizers/bpe-8192-v1
```
...
- [ ] Tokenizer uses no pretrained vocabulary/weights.
- [ ] Only permitted training text is used.
- [ ] Reload/round-trip tests pass.
- [ ] Artifact hash and vocabulary statistics are recorded.
...
3. Set `P04` to `passed`.
4. Set `P05` to `ready` only when its dependencies are passed.
