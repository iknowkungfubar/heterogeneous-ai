...
**Goal:** Create a provenance-aware relational memory and deterministic graph query layer.
...
Knowledge graphs, entities/relations, provenance edges, and traversal.
...
1. Implement deterministic graph store/query with provenance.
2. Support entity lookup, neighbors, relation filters, path/provenance traversal.
3. Define conflict/version semantics for contradictory edges.
4. Benchmark deterministic graph operations before GNNs.

## Operator commands

```bash
hai graph init
hai graph import-fixture tests/fixtures/graph.jsonl
hai graph query --subject Sacramento --predicate capital_of
hai graph self-test
```
...
- [ ] Graph operations are deterministic/tested.
- [ ] Factual edges support provenance/trust.
...
3. Set `P17` to `passed`.
4. Set `P18` to `ready` only when its dependencies are passed.
