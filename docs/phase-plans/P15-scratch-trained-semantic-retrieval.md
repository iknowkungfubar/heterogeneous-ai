# P15 — Scratch-trained semantic retrieval

**Goal:** Train a dual encoder from scratch and implement hybrid lexical+dense retrieval with provenance.
...
Embeddings, contrastive learning, vector indexes, lexical retrieval, and reranking.
...
1. Create positive/negative retrieval pairs without test leakage.
2. Train a small dual encoder from random weights.
3. Implement lexical baseline, HNSW dense retrieval, merge, and optional reranking.
4. Attach document/chunk provenance to results.
5. Compare dense, lexical, and hybrid retrieval.

## Operator commands

```bash
hai retrieval train-encoder --config configs/models/embedding.yaml
hai retrieval build-index
hai retrieval evaluate --split validation
```
...
- [ ] Scratch embeddings beat random/basic baselines.
- [ ] Hybrid retrieval evaluation exists.
...
3. Set `P15` to `passed`.
4. Set `P16` to `ready` only when its dependencies are passed.
