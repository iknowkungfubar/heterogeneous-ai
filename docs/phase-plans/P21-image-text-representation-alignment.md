# P21 — Image-text representation alignment

**Goal:** Train scratch image/text encoders into a shared representation space and evaluate retrieval.
...
Contrastive multimodal learning, shared embedding spaces, and bidirectional retrieval.
...
1. Create/validate paired image-text data with leakage-resistant splits.
2. Train image/text encoders/projectors from scratch into shared space.
3. Evaluate both retrieval directions.
4. Inspect false positives for shortcuts/spurious correlations.

## Operator commands

```bash
hai multimodal train-image-text-alignment
...
- [ ] Bidirectional retrieval exceeds random baseline.
- [ ] Alignment artifacts/metrics are reproducible.
...
3. Set `P21` to `passed`.
4. Set `P22` to `ready` only when its dependencies are passed.
