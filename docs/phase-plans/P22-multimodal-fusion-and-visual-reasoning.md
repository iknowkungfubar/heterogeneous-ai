# P22 — Multimodal fusion and visual reasoning

**Goal:** Fuse visual and language representations and benchmark objective multimodal reasoning.
...
Projection, cross-attention, latent fusion, and objective visual reasoning.
...
1. Start with simplest projection/fusion baseline.
2. Add cross-attention/shared-latent only as controlled experiments.
3. Benchmark attributes, relations, counting, matching, simple VQA.
4. Run fusion ablations and retain the simplest justified design.

## Operator commands

```bash
hai multimodal train-fusion --variant projection
hai multimodal train-fusion --variant cross-attention
hai multimodal evaluate --suite visual-objective-v1
```
...
- [ ] Multimodal reasoning beats unimodal/random baselines.
- [ ] Retained fusion is supported by ablation.
...
3. Set `P22` to `passed`.
4. Set `P23` to `ready` only when its dependencies are passed.
