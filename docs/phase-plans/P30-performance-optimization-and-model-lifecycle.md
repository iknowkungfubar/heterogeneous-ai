# P30 — Performance optimization and model lifecycle

**Goal:** Profile, lazily load, cache, quantize, and manage specialists without sacrificing measured correctness.
...
Profiling, caching, lazy loading, quantization, model registry, and quality/performance tradeoffs.
...
2. Implement model manager with load/unload/status/memory usage.
3. Add caching/lazy loading and separately versioned quantized artifacts.
4. Regression-test every optimization.
5. Define low/medium/high compute budgets.

## Operator commands

```bash
hai profile system --suite representative-v1
hai models status
hai optimization evaluate --variant lazy-loading
hai optimization evaluate --variant quantized
```
...
- [ ] Optimizations provide measured benefit without unacceptable regression.
- [ ] Canonical artifacts remain intact.
...
3. Set `P30` to `passed`.
4. Set `P31` to `ready` only when its dependencies are passed.
