# P02 — PyTorch fundamentals and GPU training smoke tests

**Goal:** Prove tensors, forward/backward passes, optimization, checkpoint basics, and GPU memory reporting work.
...
Tensors, parameters, forward pass, loss, gradients, backpropagation, optimizer step, and GPU memory basics.
...
1. Run a GPU tensor/matrix smoke test.
2. Train the included one-layer model on `y=3x+2`; observe convergence.
3. Add peak memory reporting and a checkpoint-save/load micro-test.
4. Explain the training loop in a short evidence note for the novice.
...
make env-check
make gpu-smoke
pytest -m gpu tests/smoke/test_gpu_linear.py -q
```
...
- [ ] Forward and backward passes succeed on GPU.
- [ ] Optimizer updates parameters and loss materially decreases.
- [ ] No NaN/Inf occurs.
- [ ] A basic checkpoint can be saved and reloaded.
...
3. Set `P02` to `passed`.
4. Set `P03` to `ready` only when its dependencies are passed.
