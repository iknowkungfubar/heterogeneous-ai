# P25 — Unified multimodal router and expert orchestration

**Goal:** Route text, image, audio, graph, symbolic, retrieval, and mixed tasks efficiently.
...
Multimodal task detection and cost-aware expert selection.
...
1. Extend task schema/features for text/image/audio/graph/world inputs.
2. Train/evaluate against simple modality rules.
3. Make missing/corrupt modalities structured failures instead of system crashes.
4. Measure calls and end-to-end latency.

## Operator commands

```bash
hai router build-multimodal-dataset
hai router train --mode multimodal
hai system evaluate --suite multimodal-routing-validation
```
...
- [ ] Mixed-modality routing improves utility.
- [ ] Irrelevant expensive experts are not routinely invoked.
...
3. Set `P25` to `passed`.
4. Set `P26` to `ready` only when its dependencies are passed.
