# P23 — Audio specialist from scratch

**Goal:** Train a small audio model on classification/representation tasks before speech.
...
Waveforms, features, audio encoders, and sound classification.
...
1. Create licensed audio data card and deterministic preprocessing.
2. Train CNN/Conformer/Transformer candidate from random weights on classification.
3. Measure quality/resources and choose a base encoder before speech.

## Operator commands

```bash
hai audio data-verify
hai audio train --config configs/models/audio.yaml --task sound-classification
hai audio evaluate --split validation
```
...
- [ ] Standalone audio learning is stable and beats baseline.
...
3. Set `P23` to `passed`.
4. Set `P24` to `ready` only when its dependencies are passed.
