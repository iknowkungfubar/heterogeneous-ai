# P12 — Verification layer

**Goal:** Implement deterministic and evidence-backed verification independent of model confidence.
...
Independent verification, proof vs prediction, and evidence strength.
...
1. Implement deterministic verification for substitution, Z3, graph/path checks, and sandboxed tests where applicable.
2. Define evidence-strength taxonomy.
3. Define learned/factual verification interfaces without treating raw confidence as proof.
4. Record verification failures explicitly.

## Operator commands

```bash
hai verify self-test
hai benchmark evaluate --system router-plus-verifier --split validation
```
...
- [ ] Deterministically verifiable tasks are independently checked.
- [ ] Verifier rejects plausible wrong candidates.
- [ ] Verification strength flows into consensus.
...
3. Set `P12` to `passed`.
4. Set `P13` to `ready` only when its dependencies are passed.
