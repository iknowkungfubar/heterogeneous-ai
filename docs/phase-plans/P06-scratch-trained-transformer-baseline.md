# P06 — Scratch-trained Transformer baseline

**Goal:** Train and freeze the first meaningful 25–35M parameter language-model baseline.
...
Training budgets, gradient accumulation, mixed precision, validation curves, overfitting, and experiment freezes.
...
1. Calculate exact parameter count for the baseline config.
2. Do a short preflight at intended batch/context/precision; record peak VRAM/throughput.
3. Use the documented OOM recovery order instead of silently changing architecture.
4. Run approved training with periodic validation/checkpoints.
5. Select/freeze the baseline using validation criteria only; create model card/hash.

## Operator commands

```bash
# Long run: human approval required.
hai train --experiment-config configs/experiments/TFRM-0001.yaml
hai evaluate --experiment TFRM-0001 --split validation
hai report experiment TFRM-0001
```
...
- [ ] A meaningful scratch-trained Transformer checkpoint exists.
- [ ] Config/data/tokenizer/environment are reproducible.
- [ ] Held-out metrics/resources are recorded.
- [ ] Baseline is frozen as a named canonical artifact.
...
3. Set `P06` to `passed`.
4. Set `P07` to `ready` only when its dependencies are passed.
