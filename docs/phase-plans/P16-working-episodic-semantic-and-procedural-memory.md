# P16 — Working, episodic, semantic, and procedural memory

**Goal:** Build explicit memory stores with provenance, trust, retention, and retrieval rules.
...
Working, episodic, semantic, and procedural memory; trust, retention, and poisoning controls.
...
1. Implement distinct memory schemas/stores.
2. Add source, timestamp, trust, retention, embedding and relationship links.
3. Validate candidate memories; never auto-trust model output.
4. Add duplicate/conflict handling and deletion/retention controls.
...
- [ ] Memory types are distinct/queryable.
- [ ] Untrusted input cannot silently become trusted memory.
...
3. Set `P16` to `passed`.
4. Set `P17` to `ready` only when its dependencies are passed.
