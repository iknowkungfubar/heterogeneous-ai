# P28 — Compute-aware reinforcement learning orchestration

**Goal:** Train constrained RL policies to trade verified correctness against expert/computation cost.
...
RL state/action/reward design, constrained action spaces, and compute-aware policy evaluation.
...
1. Define finite orchestration actions only.
2. Reward verified correctness, correct abstention, evidence integrity, and penalize unnecessary cost.
3. Train in a sandboxed orchestration environment.
4. Compare against static/supervised/all-expert baselines; reject RL if unjustified.

## Operator commands

```bash
hai rl train-router --environment orchestration-v1
hai rl evaluate-router --against static,xgboost,all-experts
```
...
- [ ] RL has no arbitrary shell/tool access.
- [ ] Quality/compute tradeoff is measured against simpler routers.
...
3. Set `P28` to `passed`.
4. Set `P29` to `ready` only when its dependencies are passed.
