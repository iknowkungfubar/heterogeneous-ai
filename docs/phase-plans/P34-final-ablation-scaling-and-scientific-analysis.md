# P34 — Final ablation, scaling, and scientific analysis

**Goal:** Measure component contribution, statistical uncertainty, scaling behavior, and complexity/value tradeoffs.
...
Final ablation matrices, component contribution, scaling curves, and statistical interpretation.
...
1. Freeze final candidate before opening final test.
2. Run full component combinations/removal matrix.
3. Repeat important comparisons with multiple seeds/confidence intervals where feasible.
4. Analyze compute/parameters/training-tokens vs quality and verified-correct-per-compute.
5. Conclude yes/partial/no without moving criteria after results.

## Operator commands

```bash
hai ablation run --suite final-v1 --split test
hai scaling analyze --registry experiments/registry.yaml
hai report final-science
```
...
- [ ] Final test integrity is documented.
- [ ] Each retained major component has measured contribution/engineering rationale.
- [ ] Scientific conclusion reports uncertainty/resource cost.
...
3. Set `P34` to `passed`.
4. Set `P35` to `ready` only when its dependencies are passed.
