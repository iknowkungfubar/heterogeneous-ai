# P18 — Graph neural-network specialist

**Goal:** Train a small GNN and determine whether learned relational reasoning adds value beyond graph algorithms.
...
Message passing, node/link prediction, and learned vs algorithmic relational reasoning.
...
1. Implement simple message passing in plain PyTorch.
2. Train controlled node/link/relationship tasks.
3. Compare deterministic algorithms and MLP/random baselines.
4. Analyze complementary errors before retaining the GNN.

## Operator commands

```bash
hai gnn train --config configs/models/gnn.yaml
hai gnn evaluate --split validation
hai compare graph-baseline gnn-v1
```
...
- [ ] GNN is reproducible/scratch-trained.
- [ ] Contribution beyond graph baselines is quantified.
- [ ] No-value result may retire it.
...
3. Set `P18` to `passed`.
4. Set `P19` to `ready` only when its dependencies are passed.
