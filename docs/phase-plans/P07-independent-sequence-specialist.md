# P07 — Independent sequence specialist

**Goal:** Train a structurally different GRU sequence model and quantify complementary errors.
...
GRUs/recurrent networks and why error diversity matters more than secondary-model headline accuracy.
...
1. Implement a GRU model in plain PyTorch with the same tokenizer and comparable corpus.
2. Train/evaluate independently.
3. Create paired both-correct/Transformer-only/GRU-only/both-wrong analysis.
4. Quantify complementarity by task category.

## Operator commands

```bash
hai train --model configs/models/gru.yaml --experiment GRU-0001
hai evaluate --experiment GRU-0001 --split validation
hai compare-errors --left TFRM-0001 --right GRU-0001
```
...
- [ ] GRU training/evaluation is reproducible.
- [ ] No pretrained weights are used.
- [ ] Error-complementarity analysis exists.
- [ ] Negative findings remain documented.
...
3. Set `P07` to `passed`.
4. Set `P08` to `ready` only when its dependencies are passed.
