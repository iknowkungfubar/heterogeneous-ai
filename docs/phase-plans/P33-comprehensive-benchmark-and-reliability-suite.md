# P33 — Comprehensive benchmark and reliability suite

**Goal:** Evaluate every supported domain, uncertainty behavior, fault injection, and efficiency dimension.
...
Comprehensive reliability measurement and domain-specific metrics.
...
1. Create domain suites for every supported subsystem.
2. Add typos, irrelevant/contradictory context, OOD, missing/corrupt modality, retrieval/expert failure perturbations.
3. Produce domain-by-domain reliability/robustness/efficiency reports without hiding regressions in one aggregate.

## Operator commands

```bash
hai evaluation run --suite comprehensive-v1 --split validation
hai evaluation robustness --suite robustness-v1
hai evaluation efficiency --suite representative-v1
```
...
- [ ] All supported domains have appropriate metrics.
- [ ] Reliability, robustness, and efficiency reports are complete.
...
3. Set `P33` to `passed`.
4. Set `P34` to `ready` only when its dependencies are passed.
