# P11 — Learned task router

**Goal:** Train an interpretable router that selects only useful specialists for each task.
...
Supervised routing, feature engineering, cost-aware selection, and avoiding unnecessary experts.
...
1. Generate a TRAIN-only meta-dataset from benchmark tasks/expert outcomes.
2. Implement rule/interpretable ML baselines before complex routing.
3. Evaluate downstream task utility and expert-call cost, not router accuracy alone.
4. Freeze validation-selected config without final-test tuning.

## Operator commands

```bash
hai router build-dataset --benchmark core-objective-v1
hai router train --config configs/routing/default.yaml --split train
hai router evaluate --split validation
```
...
- [ ] Router uses only allowed training data.
- [ ] It improves/preserves utility vs simple baselines.
- [ ] Feature/decision traces are inspectable.
...
3. Set `P11` to `passed`.
4. Set `P12` to `ready` only when its dependencies are passed.
