...
**Goal:** Train a transition/world model in controlled environments and measure rollout error.
...
States/actions, transition models, one-step vs rollout error, and latent prediction.
...
1. Implement deterministic GridWorld simulator/dataset generator.
2. Train scratch transition model on `(state_t, action_t) -> state_t+1`.
3. Measure one-step and multi-step rollout error.
4. Add latent/JEPA-style prediction only as a later controlled variant.
5. Mark outputs as simulations in provenance.

## Operator commands

```bash
hai world generate-gridworld-data
hai world train --config configs/models/world.yaml
hai world evaluate-rollouts --horizons 1,2,5,10
```
...
- [ ] World model beats trivial transition baseline.
- [ ] Rollout error by horizon is quantified.
...
3. Set `P26` to `passed`.
4. Set `P27` to `ready` only when its dependencies are passed.
